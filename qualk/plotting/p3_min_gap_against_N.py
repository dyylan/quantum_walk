...
def p3(start_N, end_N, opt_gammaNs, alpha, marked, ring, lat_d, step_N=1):
    dimensions = list(range(start_N, end_N+1, step_N))
    min_gaps = []
    for N in dimensions:
        gamma = lookup_gamma(opt_gammaNs, N)
        H = Hamiltonian(N, gamma, alpha, marked, ring, lat_d)
...
def run():
    ...
    dimensions, min_gaps = p3(start_N, end_N, optimum_gammaNs, alpha, marked_state, ring, lattice_dimension, step_N)

...
def optimum_time(lattice_d, dimensions, gamma, start_time, end_time, mark, alpha, number_of_points):
...
if __name__ == "__main__":
    ...
    optimum_gammaNs_file = f'optimum_gamma/alpha={alpha}/optimum_gammaNs.csv'
    ...
    for N in dimensions:
        ...
        opt_time, max_fidelity = optimum_time(lattice_d, N, gamma, start_time, end_time, marked_state, alpha, number_of_points)
    ...
    if lattice_d == 1:
        optimum_times_df.to_csv(f'optimum_time/alpha={alpha}/optimum_times_m={marked_state}.csv', index=False)
    elif lattice_d == 2:
